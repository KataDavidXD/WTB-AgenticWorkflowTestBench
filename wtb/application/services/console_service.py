...
class WorkspaceTracker(SqliteFileTrackingService):
    ...
    def restore_from_checkpoint(self, checkpoint_id):
        ...
        existing = {
            p.resolve(): p.read_bytes() for p in self.output.rglob("*") if p.is_file()
        }
        try:
        ...
        return FileRestoreResult(
            ...
            total_size_bytes=sum(len(c) for _, c in prepared),
            restored_paths=[str(p) for p, _ in prepared],
            success=True,
        )
...
class ConsoleService:
    def __init__(self, data_dir, config_path=None):
        ...
        entries = [
            {
                "factory": "wtb.testing.console_project:create_project",
                "initial_state": {"text": "WTB", "repeat": 2, "delay": 0.2},
            }
        ]
        if config_path:
        ...
        for e in self.store.list("execution"):
            if e["status"] in {"queued", "running", "pausing", "stopping"} or e.get(
                "pendingOperation"
            ):
                e.update(
                    status="failed",
                    error="服务在执行期间退出；可从最后有效检查点回退",
                    activeNodes=[],
    ...
    def emit(self, execution_id, kind, message):
        event = dict(
            id=uid(), executionId=execution_id, kind=kind, message=message, time=now()
        )
    ...
    def catalog(self):
        ...
        for name, (p, initial) in self.projects.items():
            node_variant_details = {
                node: [item.to_dict() for item in choices.values()]
                for node, choices in p._node_variants.items()
            }
            workflow_variant_details = {
                variant_name: item.to_dict()
                for variant_name, item in p._workflow_variants.items()
            }
            projects.append(
                dict(
                    id=name,
            ...
            choices += [
                (f"{node}:{v}", None, {node: v})
                for node, vs in p.list_variants().items()
                for v in vs
            ]
            for label, workflow_variant, node_variants in choices:
                graph = p.build_graph(node_variants, workflow_variant)
                native = getattr(p, "runtime_backend", "langgraph") == "native"
                if native:
                    if not isinstance(
                        getattr(graph, "nodes", None), dict
                    ) or not getattr(graph, "entry_point", None):
                        raise ValueError(
                            "Native console projects require nodes, edges, and entry_point"
                        )
                    g = SimpleNamespace(
                        nodes={
                            key: SimpleNamespace(id=key, name=key)
                        ...
                        edges=[
                            SimpleNamespace(
                                source=edge[0],
                                target=edge[1],
                                conditional=bool(edge[2] if len(edge) > 2 else None),
                                data=edge[2] if len(edge) > 2 else None,
                            )
                ...
                for i, n in enumerate(g.nodes.values()):
                    ...
                    nodes.append(
                        dict(
                            ...
                            implementation=node_variants.get(n.id, n.name),
                            x=(i % 4) * 240,
                            y=(i // 4) * 170,
                        )
                    )
                    detail = next(
                        (
                            item
                            for item in node_variant_details.get(n.id, [])
                            if item["name"] == node_variants.get(n.id)
                        ),
                        None,
                    )
                    components.append(
                        dict(
                            ...
                            if detail
                            else p.execution.get_node_resources(n.id).to_dict(),
                        )
                    )
                variants.append(
                    dict(
                        ...
                        nodeVariants=node_variants,
                        runtimeBackend="native" if native else "langgraph",
                        entryPoint=getattr(graph, "entry_point", None),
                        capabilities={
                            "pause": True,
                            "resume": True,
                            "rollback": True,
    ...
    def context(self, e):
        ...
        if e.get("runtimeBackend", "langgraph") == "langgraph":
            config.state_adapter_mode = "langgraph"
        config.log_sql = False
        tracker = WorkspaceTracker(e["storeDir"], e["runtime"]["output"])
        managed = ExecutionControllerFactory(config).create_isolated(
            file_tracking_service=tracker, output_dir=e["runtime"]["output"]
        )
        try:
            with managed:
    ...
    def start(
        self, variant_id, state, breakpoints=None, node_variants=None, batch_id=None
    ):
        if self.closed:
        ...
        e = dict(
            ...
            createdAt=now(),
            state=state,
            runtimeBackend=variant["runtimeBackend"],
            ...
            elapsedMs=0,
            storeDir=str(workspace),
            graphPath=str(workspace / "graph.pkl"),
            batchId=batch_id,
            parentId=None,
            fromCheckpointId=None,
            runtime=self.runtime(workspace),
            pendingOperation=None,
            control=None,
            attempts=[
                dict(id=uid(), kind="initial", checkpointId=None, createdAt=now())
            ],
        )
        with self.context(e) as (ctrl, uow, _):
            if e["runtimeBackend"] == "native":
                workflow = TestWorkflow(
                    id=p.id, name=p.name, entry_point=variant["entryPoint"]
                )
                for node in variant["nodes"]:
                    workflow.add_node(
                        WorkflowNode(
                            ...
                            name=node["id"],
                            type="action",
                            config={"_wtb_native_callable_required": True},
                        )
                    )
            ...
            ctrl.create_execution(
                workflow, state, breakpoints=breakpoints or [], execution_id=id
            )
        self.store.put("execution", e)
    ...
    def submit(self, id, action, payload):
        with self.lock:
            ...
            op = dict(
                id=uid(), executionId=id, type=action, status="queued", createdAt=now()
            )
            self.store.put("operation", op)
    ...
    def command(self, id, action, payload):
        with self.lock:
            e = self.store.get("execution", id)
            if action in {"pause", "stop"} and e["status"] in {
                "running",
                "queued",
                "pausing",
            }:
                ...
                self.update(
                    id,
                    control=action,
                    controlOperation=op["id"],
                    status="pausing" if action == "pause" else "stopping",
                )
                self.emit(id, "control", action + " requested")
                return dict(executionId=id, operationId=op["id"])
            if e["status"] in {"running", "queued", "pausing", "stopping"} or e.get(
                "pendingOperation"
            ):
                raise Conflict("请等待执行到达暂停边界")
            if (
                action in {"resume", "edit", "checkpoint", "breakpoints"}
                and e["status"] != "paused"
            ):
    ...
    def boundary(self, id, ctrl, tracker, snapshot):
        ...
        self.record_checkpoint(
            ...
            state,
            list(snapshot.next),
            (snapshot.metadata or {}).get("step", 0),
            snapshot.created_at,
            (snapshot.parent_config or {}).get("configurable", {}).get("checkpoint_id"),
        )
    ...
    def record_checkpoint(
        ...
        cp = dict(
            ...
            nodeRuns=[
                r
                for r in current["nodeRuns"]
                if r["id"]
                in current.get("activeRunIds", [n["id"] for n in current["nodeRuns"]])
            ],
            step=step,
            fileCommitId=tracked.commit_id,
            tracked=len(paths),
            parentCheckpointId=parent_id,
            attemptId=(current.get("attempts") or [{}])[-1].get("id"),
        )
        cp["completedNodes"] = list(
            dict.fromkeys(
                r["nodeId"] for r in cp["nodeRuns"] if r["status"] == "completed"
            )
        )
        cp["lastNode"] = next(
            (
                r["nodeId"]
                for r in reversed(cp["nodeRuns"])
        ...
        self.update(
            id, checkpointId=cp_id, state=state, nextNodes=next_nodes, activeNodes=[]
        )
    ...
    def node_event(self, id, event):
        ...
        with self.lock:
            ...
            if event["type"] == "task":
                ...
                run = next(
                    (
                        ...
                        if r["taskId"] == p["id"] and r["status"] == "running"
                    ),
                    None,
                )
                if run:
                    run.update(
                        status="failed" if p.get("error") else "completed",
                        error=p.get("error"),
                        elapsedMs=int(
                            (
            ...
            self.update(
                id,
                nodeRuns=runs,
                activeRunIds=e.get("activeRunIds", []),
                activeNodes=[r["nodeId"] for r in runs if r["status"] == "running"],
            )
    ...
    def work(self, id, op_id, action, payload):
        ...
        try:
            ...
            with self.context(e) as (ctrl, uow, tracker):
                ...
                if e.get("runtimeBackend") == "native":
                    from wtb.application.services.console_native import (
                        NativeConsoleRuntime,
                    )

                    execution = NativeConsoleRuntime(self, ctrl, uow, tracker).work(
                        e, graph, action, payload, op
                    )
                    if action != "fork":
                        self.update(
                            id,
                            status=execution.status.value,
                            state=execution.state.workflow_variables,
                ...
                adapter.configure_runtime(
                    event=lambda event: self.node_event(id, event),
                ...
                if action in {"run", "resume"}:
                    if e.get("control") == "stop":
                        ...
                        execution = (
                            ctrl.run(id, graph)
                            if action == "run"
                            else ctrl.resume(id, payload.get("state"))
                        )
                    ...
                    child_tracker = WorkspaceTracker(
                    ...
                    try:
                        ...
                        self.boundary(
                            ...
                            ctrl,
                            child_tracker,
                            adapter.get_compiled_graph().get_state(
                                adapter.get_config()
                            ),
                        )
                    ...
                    new_config = adapter.get_compiled_graph().update_state(
                        config, payload.get("state", {})
                    )
                    ...
                    execution.checkpoint_id = snap.config["configurable"][
                        "checkpoint_id"
                    ]
            ...
            if e.get("controlOperation"):
                ...
                control.update(
                    status="completed"
                    if e["status"] in {"paused", "cancelled"}
                    else "failed",
                    finishedAt=now(),
                )
    ...
    def files(self, id, checkpoint_id, path=None):
        ...
        with WorkspaceTracker(e["storeDir"], e["runtime"]["output"]) as tracker:
            ...
            if path is None:
                return [
                    dict(
                        path=str(tracker.relative(f)).replace("\\", "/"),
                        hash=f.file_hash,
                        size=f.size_bytes,
                    )
                    for f in files
            ...
            file = next(
                (
                    f
                    for f in files
                    if str(tracker.relative(f)).replace("\\", "/") == path
                ),
                None,
            )
            if not file:
    ...
    def system(self):
        return dict(
            ...
            cache=None,
            outbox=None,
            integrity=self.store.list("integrity"),
    ...
    def check_integrity(self):
        ...
        for cp in self.store.list("checkpoint"):
            try:
                ...
                with WorkspaceTracker(e["storeDir"], e["runtime"]["output"]) as tracker:
                    if (
                        tracker.get_commit_for_checkpoint(cp["checkpointId"])
                        != cp["fileCommitId"]
                    ):
