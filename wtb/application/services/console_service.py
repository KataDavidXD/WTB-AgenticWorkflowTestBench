...
class ConsoleService:
    ...
    def catalog(self):
        ...
        for name, (p, initial) in self.projects.items():
            node_variant_details = {node: [item.to_dict() for item in choices.values()] for node, choices in p._node_variants.items()}
            workflow_variant_details = {variant_name: item.to_dict() for variant_name, item in p._workflow_variants.items()}
            projects.append(dict(id=name, name=name, description=p.description, version=p.version, initialState=initial, nodeVariants=p.list_variants(), nodeVariantDetails=node_variant_details, workflowVariantDetails=workflow_variant_details, sdk=dict(fileTracking=p.file_tracking.to_dict(), environment=p.environment.to_dict(), execution=p.execution.to_dict(), workspaceIsolation=p.workspace_isolation.to_dict(), pauseStrategy=p.pause_strategy.to_dict())))
            ...
            for label, workflow_variant, node_variants in choices:
                ...
                for i, n in enumerate(g.nodes.values()):
                    ...
                    components.append(dict(id=component_id, name=n.name, description=detail["description"] if detail else "已注册图节点", version=None, tags=[], environment=detail["environment"] if detail else None, resources=detail["resources"] if detail else p.execution.get_node_resources(n.id).to_dict()))
