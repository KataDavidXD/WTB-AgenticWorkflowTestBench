        ...
        {[
          ...
          return (
            <div key={i}>
              ...
              {v && (
                <>
                  <WorkflowCanvas model={v} compact />
                  <JsonView value={v.nodeVariants} />
                </>
              )}
            </div>
          );
        ...
        <tbody>
          {variants.map((v) => (
            <tr key={v.id}>
              <td>{v.id}</td>
              <td>
                {v.nodes.map((n) => `${n.id}: ${n.implementation}`).join(" / ")}
              </td>
            </tr>
          ))}
        </tbody>
...
export function BatchPage({
  ...
  const [inputs, setInputs] = useState(
    '[{"text":"A","repeat":1},{"text":"B","repeat":2}]',
  );
  ...
  const batch =
    batches.data?.find((b) => b.id === selected) ?? batches.data?.[0];
  const results = useRemote<Page<Execution>>(
    batch ? `/batch-tests/${batch.id}/results` : undefined,
    revision,
  );
  const submit = async () => {
    setBusy(true);
    try {
      ...
      if (
        ...
        parsed.some((s) => !s || typeof s !== "object" || Array.isArray(s))
      )
        throw new Error("输入必须是 JSON 对象数组");
      const b = await api<Batch>("/batch-tests", {
        variants: chosen,
        inputs: parsed,
      });
      setSelected(b.id);
      setMessage("已提交真实批量任务");
    } catch (e) {
      setMessage(String(e));
    } finally {
      setBusy(false);
  ...
  const completed =
    results.data?.items.filter((e) => e.status === "completed") ?? [];
  const terminal =
    results.data?.items.filter((e) =>
      ["completed", "failed", "cancelled"].includes(e.status),
    ) ?? [];
  return (
    <section className="panel panel-body">
      <h2>本地批量实验</h2>
      <p>最多两个任务同时运行。质量分数：未评估。</p>
      {catalog.variants.map((v) => (
        <label key={v.id} style={{ marginRight: 16 }}>
          <input
            type="checkbox"
      ...
      <p>
        完成 {terminal.length} / {batch?.executionIds.length ?? 0} ·
        已结束任务成功率{" "}
        {terminal.length
          ? ((completed.length / terminal.length) * 100).toFixed(0) + "%"
          : "—"}
      </p>
...
export function SystemPage({ revision }: { revision: number }) {
  ...
  return (
    <section className="panel panel-body">
      <h2>系统信息</h2>
      <p>本机服务 · SQLite · 真实 CAS。未采集的数据不估算。</p>
      ...
      <button
        ...
        onClick={() => {
          setBusy(true);
          void api("/system/integrity", {})
            .then(() => setMessage("检查已完成"))
