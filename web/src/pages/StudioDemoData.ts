...
export interface StudioVariant { id: string; name: string; label: string; cfg: Configuration[]; links: [NodeId, NodeId][]; extra: [NodeId, NodeId][]; tags: string[]; }
...
export function configureStudioCatalog(source: ApiVariant[]) {
  ...
  variants = source.map(variant => {
    ...
    const ordered = COMPONENTS.filter(component => present.has(component.id)).map(component => component.id);
    const sequential = new Set(ordered.slice(1).map((node, index) => `${ordered[index]}>${node}`));
    const links = variant.edges
      .filter(edge => !edge.source.startsWith('__') && !edge.target.startsWith('__'))
      .map(edge => [edge.source, edge.target] as [NodeId, NodeId]);
    return {
      id: variant.id,
      name: variant.name,
      label: variant.description || variant.workflowVariant || '默认工作流',
      cfg: COMPONENTS.map(component => variant.nodes.find(node => node.id === component.id)?.implementation || null),
      ...
      links,
      extra: links.filter(([source, target]) => source === target || !sequential.has(`${source}>${target}`)),
...
export const edges = (variant: StudioVariant): [NodeId, NodeId][] => variant.links;
