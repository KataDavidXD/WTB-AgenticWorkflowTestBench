...
export function StudioPageFrame({ children }: { children: ReactNode }) {
  const { state, dispatch, saved, drawer, modal, setModal, setDrawer, notify } = useStudio();
  useEffect(() => {
    const keydown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { setDrawer(false); return; }
      const target = e.target as HTMLElement;
      if (['INPUT', 'SELECT', 'TEXTAREA'].includes(target.tagName) || modal || drawer) return;
      if (target.closest('[role="button"]') || (target.tagName === 'BUTTON' && ['Enter', ' '].includes(e.key))) return;
      if (['1', '2', '3', '4'].includes(e.key)) dispatch({ type: 'view', view: projections[Number(e.key) - 1].id });
      else if (e.key === ' ') { e.preventDefault(); dispatch({ type: 'pause' }); }
      else if (e.key.toLowerCase() === 'f') setModal('fork');
      else if (e.key.toLowerCase() === 'r') { if (['running', 'pausing', 'idle'].includes(selectedRun(state).status)) notify('先暂停到节点边界，再执行回退。'); else setModal('rollback'); }
