...
export function loomGeometry() {
  ...
  const xs = COMPONENTS.map((_, i) => 217 + i * gap), seats: Record<string, number> = {};
  ...
  const pos = (v: StudioVariant, i: number) => ({ x: xs[i], y: seats[`${COMPONENTS[i].id}:${v.cfg[i]}`] + (row(v) - (variants.length - 1) / 2) * 3.05 });
  const path = (v: StudioVariant) => { const y = 78 + row(v) * 36, points = [{ x: 138, y }, ...nodes(v).map(node => pos(v, COMPONENTS.findIndex(c => c.id === node))), { x: 1206, y }]; return points.reduce((s, b, i) => { if (!i) return `M ${b.x} ${b.y}`; const a = points[i - 1], m = (a.x + b.x) / 2; return s + ` C ${m} ${a.y}, ${m} ${b.y}, ${b.x} ${b.y}`; }, ''); };
...
export function PathLoomConfigurationSlots({ geometry }: { geometry: Geometry }) {
  ...
  return <>{COMPONENTS.map((c, i) => { const x = geometry.xs[i], selected = state.selectedNode === c.id; return <Fragment key={c.id}>
    ...
    <Hit label={`${c.id} ${c.name}；查看当前变体的节点上下文`} onClick={() => dispatch({ type: 'node', id: c.id })}><circle className="hover-stroke" cx={x} cy={18} r={13} fill={selected ? 'var(--ink)' : 'var(--surface)'} stroke="var(--line)" /><Mono x={x} y={22} size={11} color={selected ? 'var(--bg)' : 'var(--ink)'} textAnchor="middle">{c.id}</Mono><Mono x={x - 13} y={44} size={8}>{c.en}</Mono></Hit>
...
export function PathLoomVariantTrack({ v, geometry }: { v: StudioVariant; geometry: Geometry }) {
  ...
  return <><path d={path} fill="none" stroke={sel ? 'var(--accent)' : 'var(--muted)'} strokeWidth={sel ? 3 : 1.15} opacity={sel ? 1 : state.focus ? .065 : .25} className="track-path" />
    ...
    {sel && nodes(v).map(id => { const i = COMPONENTS.findIndex(c => c.id === id), c = COMPONENTS[i], p = geometry.pos(v, i), ctx = nodeContext(state, c.id); return <Fragment key={c.id}><Hit label={`${c.id} ${c.name} · ${LABELS[String(v.cfg[i])]} · ${NODESTATUS[ctx.ns]}`} onClick={() => dispatch({ type: 'node', id: c.id })}><circle cx={p.x} cy={p.y} r={c.id === state.selectedNode ? 7 : 4.5} fill={['queued', 'paused'].includes(ctx.ns) ? 'var(--bg)' : 'var(--accent)'} stroke={['queued', 'paused'].includes(ctx.ns) ? 'var(--accent)' : 'var(--bg)'} strokeWidth={2} /><circle cx={p.x} cy={p.y} r={14} fill="transparent" /></Hit>{ctx.ns === 'running' && <circle className="pulse" cx={p.x} cy={p.y} r={11} fill="none" stroke="var(--accent)" strokeWidth={2} />}</Fragment>; })}
...
export function PathLoomSelectedTopology() {
  ...
  return <><line x1={0} y1={418} x2={1360} y2={418} stroke="var(--line)" /><Mono x={0} y={443} size={8}>SELECTED TOPOLOGY</Mono>
    {edges(v).map(([a, b]) => { const ia = ns.indexOf(a), ib = ns.indexOf(b); if (ia < 0 || ib < 0) return null; const xa = x + ia * gap, xb = x + ib * gap, extra = ia === ib || ib - ia > 1; const d = ia === ib ? `M${xa},${y - 10} C${xa + 34},${y - 48} ${xa - 34},${y - 48} ${xa},${y - 10}` : extra ? `M${xa},${y} Q${(xa + xb) / 2},${y - 35} ${xb},${y}` : `M${xa + 11},${y} L${xb - 12},${y}`; return <path key={a + b} d={d} fill="none" stroke={extra ? 'var(--accent)' : 'var(--line)'} strokeWidth={1.2} markerEnd="url(#arrowhead)" />; })}
...
export function PathLoomStage() {
  ...
  return <StudioSvg label="组件配置路径织谱"><rect x={166} y={50} width={1004} height={353} fill="url(#dots)" /><Mono x={0} y={28} size={9}>VARIANTS / {variants.length}</Mono><Mono x={1220} y={28} size={9}>LATEST RUN</Mono><PathLoomConfigurationSlots geometry={geometry} />
