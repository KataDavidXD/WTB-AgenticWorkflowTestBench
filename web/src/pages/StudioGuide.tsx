export function StudioGuide({ onClose }: { onClose: () => void }) {
 ...
 <p>这里不是四套数据，也不是四张不同配色的流程图。四个视角共用同一组变体、运行、检查点与资源关系，只选择不同的数学投影。所有路径、资源和状态均为演示数据。</p>
 ...
 <tr><td>变体图谱</td><td>两个离散配置维度组成平面；六段环表示 A–F 的使用 / 缺席；外环表示状态。</td><td>查看实验覆盖与选定两项比较。点间距离无连续数值意义，不是假装有依据的 embedding。</td></tr>
 </tbody></table>
 <h3>交互约定</h3><p>点击变体或谱线切换运行；点击节点切换执行上下文；下方小圆点选择已经持久化的检查点，不会自动回退。暂停 / 继续、推进一步、回退与 Fork 会同步到四个视图。灰色圆点表示尚未生成的检查点。所有操作仅修改浏览器中的示例状态，重置可恢复；资源详情中的路径可复制。</p>
